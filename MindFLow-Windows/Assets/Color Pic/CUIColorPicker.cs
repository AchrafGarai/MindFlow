...
public class CUIColorPicker : MonoBehaviour
{
    ...
    private void Setup( Color inputColor )
    {
        ...
        hueTex.Apply();
		hueGO.GetComponent<UnityEngine.UI.Image>().sprite = Sprite.Create( hueTex, new Rect( 0, 0.5f, 1, 6 ), new Vector2( 0.5f, 0.5f ) );
        var hueSz = GetWidgetSize( hueGO );
        var satvalTex = new Texture2D(2,2);
		satvalGO.GetComponent<UnityEngine.UI.Image>().sprite = Sprite.Create( satvalTex, new Rect( 0.5f, 0.5f, 1, 1 ), new Vector2( 0.5f, 0.5f ) );
        ...
        Action applySaturationValue = () => {
			...
			var resImg = result.GetComponent<UnityEngine.UI.Image>();
